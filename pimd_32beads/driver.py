...
class BaseDriver(object):
    ...
    def getforce(self):
        ...
        # virial = np.diag((self.force * self.crd).sum(axis=0) - self.virial/3).ravel() / EH
        # virial = np.diag((self.force * self.crd).sum(axis=0) - self.virial*L).ravel() / EH
        virial = self.virial / EH
        for v in virial:
